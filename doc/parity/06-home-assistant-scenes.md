...
Scenes are listed in the Bifrost inventory as hidden-by-default Home Assistant entities. Add a
scene to a room and save the layout to expose it to Hue; this keeps a large HA scene collection
from unexpectedly filling the bridge.
...
- A scene with no target list is still imported once explicitly enabled and remains recallable
  through HA.
- An `unknown` HA scene state is still considered recallable; only `unavailable` is treated as
  unavailable.
